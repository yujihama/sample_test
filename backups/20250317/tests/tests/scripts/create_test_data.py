    ...
    with session_scope() as session:
        for proc in procedures:
            proc_obj = {
                ...
                "risk_areas": json.dumps(proc["risk_areas"], ensure_ascii=False),
                "required_data_fields": json.dumps(proc["required_data_fields"], ensure_ascii=False),
...
EXP015,2024-03-25,佐藤健太,技術部,9000,消耗品購入,加藤部長,2024-03-26,承認済,"""
                ...
                sample_obj = {
                    ...
                    "row_count": 15,
                    "columns": json.dumps(["申請ID", "申請日", "申請者", "部門", "金額", "目的", "承認者", "承認日", "ステータス", "備考"], ensure_ascii=False),
                    "file_metadata": json.dumps({"description": "全部門の経費申請と承認状況のサンプルデータ"}, ensure_ascii=False),
                    "created_at": datetime.datetime.now()
                }
...
INV015,ヘッドフォン O型,周辺機器,25,25,9000,東京倉庫,2024-03-15,田中"""
                ...
                sample_obj = {
                    ...
                    "content_type": "text/csv",
                    "row_count": 15,
                    "columns": json.dumps(["商品ID", "商品名", "カテゴリ", "在庫数", "理論在庫", "単価", "倉庫", "最終更新日", "担当者"], ensure_ascii=False),
                    "file_metadata": json.dumps({"description": "主要倉庫の在庫記録と実在庫のサンプルデータ"}, ensure_ascii=False),
                    "created_at": datetime.datetime.now()
                }
                created = repo.create(session, sample_obj)
                sample_data_list.append({
                ...
                for sample in procedure_samples[procedure["id"]]:
                    ...
                    for agent_id in [settings.AGENT_A_ID, settings.AGENT_B_ID, settings.AGENT_C_ID, settings.AGENT_D_ID]:
                        ...
                        agent_state_obj = {
                            ...
                            "status": agent_status,
                            "progress": progress,
                            "context": json.dumps({"initialized": True}, ensure_ascii=False),
