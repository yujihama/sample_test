...
class ImageProcessor(ToolBase):
    ...
    def __init__(self, tool_id: Optional[str] = None):
        super().__init__(tool_id or "image_processor")
...
class ExcelProcessor(ToolBase):
    ...
    def __init__(self, tool_id: Optional[str] = None):
        super().__init__(tool_id or "excel_processor")
...
class ToolRegistry:
    ...
    def _register_test_tools(self):
        ...
        self.register_tool(image_processor)
        
        # Excel処理ツール
        excel_processor = ExcelProcessor()
        self.register_tool(excel_processor)
    ...
    def get_tool_by_name(self, tool_name: str) -> Optional[ToolBase]:
        """
        ツール名でツールを取得
        ...
        Returns:
            Optional[ToolBase]: ツールインスタンス
        """
        for tool_id, tool in self.tools.items():
            if tool.__class__.__name__ == tool_name:
                return tool
        
        logger.warning(f"ツール名 {tool_name} に一致するツールが見つかりません")
        return None
