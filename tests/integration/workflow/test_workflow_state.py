...
class TestWorkflowState(unittest.TestCase):
    ...
    def test_compress_and_decompress_workflow_state(self):
        ...
        logger.info(f"圧縮前のサイズ: {len(json_utils.json_serialize(self.workflow_state))} バイト")
        logger.info(f"圧縮後のサイズ: {len(compressed_state)} バイト")
        
        # 圧縮されたことを確認
        self.assertLess(len(compressed_state), len(json_utils.json_serialize(self.workflow_state)))
    ...
    def test_agent_a_node(self, mock_sample_repo_class, mock_analyze):
        ...
        logger.info(f"Agent Aノード実行結果: {json_utils.json_serialize(result['agents']['agent_a']['data'])}")
